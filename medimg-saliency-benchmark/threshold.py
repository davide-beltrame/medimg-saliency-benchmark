...
ANNOTATIONS_METADATA_PATH = os.path.join(
    PROJECT_ROOT, "data/annotations/clean_metadata.csv"
)
...
def get_expert_consensus_masks_for_specific_annotators(
    group_name, filtered_annotations_metadata
):
    """
    ...
    unique_image_names_for_this_consensus = sorted(
        list(set(record["image_name"] for record in filtered_annotations_metadata))
    )

    if not unique_image_names_for_this_consensus:
        print(
            f"    No images found with annotations from group '{group_name}' after filtering."
        )
        return {}
    ...
    for image_name in unique_image_names_for_this_consensus:
        ...
        if (
            not base_processed_masks
        ):  # If all individual masks processed to empty or were initially empty
            continue
    ...
    print(
        f"    Generated {len(expert_consensus_masks)} non-empty consensus masks for group '{group_name}'."
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate IoU vs. Binarization Threshold for multiple models."
    )
    parser.add_argument(
        "saliency_method", type=str, help="Saliency method (e.g., 'CAM', 'GradCAM')."
    )
    ...
    annotations_metadata_list_filtered = pd.read_csv(
        os.path.join(os.path.dirname(ANNOTATIONS_METADATA_PATH), "clean_metadata.csv")
    ...
    consensus_masks_full = get_expert_consensus_masks_for_specific_annotators(
        "Full Expert", annotations_full_expert
    )

    all_evaluation_image_names = sorted(
        list(set(r["image_name"] for r in annotations_non_test_list))
    )
    print(
    ...
    for model_key, ckpt_filename_part in MODEL_CONFIGS.items():
        ...
        if not os.path.exists(target_ckpt_path):
            print(
                f"Error: Checkpoint file '{ckpt_filename_part}' for model '{model_key}' not found at {target_ckpt_path}. Skipping this model."
            )
            continue

        print(f"  Attempting to load checkpoint: {ckpt_filename_part}")
        model_checkpoint = BaseCNN.load_from_checkpoint(
            target_ckpt_path, map_location=device
        )
        ...
        if saliency_method_upper == "CAM":
            if hasattr(model_to_explain, "linear") and not model_to_explain.linear:
                print(
                    f"  Warning: CAM selected, loaded model '{type(model_to_explain).__name__}' has linear=False. CAM may not work as expected."
                )
            ...
            print(
                f"  Error: Saliency method '{saliency_method_upper}' not supported. Skipping saliency for this model."
            )
            continue
        print(
            f"  Initialized saliency method: {saliency_method_upper} for {model_key.upper()}"
        )
        ...
        print(
            f"  Starting IoU calculation for {model_key.upper()} across {len(all_evaluation_image_names)} images..."
        )
        for image_filename in all_evaluation_image_names:
            image_path_pneumonia = os.path.join(
                ORIGINAL_IMAGES_DIR_FOR_SALIENCY, "PNEUMONIA", image_filename
            )
            image_path_normal = os.path.join(
                ORIGINAL_IMAGES_DIR_FOR_SALIENCY, "NORMAL", image_filename
            )
            ...
            if saliency_map_np.shape != MODEL_INPUT_SIZE:
                saliency_map_np = utils.cv2.resize(
                    saliency_map_np,
                    MODEL_INPUT_SIZE,
                    interpolation=utils.cv2.INTER_LINEAR,
                )

            if image_filename in consensus_masks_full:
                images_contributing_full_model.add(image_filename)
                for t in thresholds_arr:
                    bin_map = utils.binarize_saliency_map(
                        saliency_map_np, threshold_value=t
                    )
                    iou_results_full_model[t].append(
                        utils.calculate_iou(
                            bin_map, consensus_masks_full[image_filename]
                        )
                    )

        if hasattr(saliency_tool, "remove_hook") and callable(
            saliency_tool.remove_hook
        ):
            saliency_tool.remove_hook()
        if hasattr(saliency_tool, "remove_hooks") and callable(
        ...
        avg_iou_full_model = [
            np.mean(iou_results_full_model[t]) if iou_results_full_model[t] else np.nan
            for t in thresholds_arr
        ]
        ...
        if n_full > 0:
            ...
            if not pd.Series(avg_iou_full_model).isnull().all():
                ...
                best_threshold_details.append(
                    f"  {model_key.upper()} Full: Best Thr={best_thresh:.4f}, Max IoU={max_iou:.4f} (n={n_full})"
                )
    ...
    best_thresholds_df.to_csv(
        os.path.join(PLOTS_DIR, f"best_thresholds_{saliency_method_upper.lower()}.csv"),
        index=False,
    )
    print(
    ...
    for i, model_key in enumerate(MODEL_CONFIGS.keys()):
        ...
        if (
            n_full > 0
            and col_full in results_df.columns
            and not results_df[col_full].isnull().all()
        ):
            plt.plot(
                results_df["Saliency Binarization Threshold"],
                results_df[col_full],
                marker="o",
                markersize=4,
                linestyle="-",
                label=f"{model_key.upper()} Full",
